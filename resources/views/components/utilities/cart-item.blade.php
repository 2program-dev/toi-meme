...
<div class="flex flex-wrap flex-col md:flex-row">
    <div @class([
        ...
        <img src="{{ Storage::get($cartItem->product->image) }}" class="size-full object-contain aspect-square">
    </div>
        ...
        @if($cartItem->product->min_qty_for_customization && $cartItem->quantity >= $cartItem->product->min_qty_for_customization)
            <div class="flex items-center">
                ...
                <span class="ml-3 text-sm" id="customization-{{ $cartItem->id }}-label">
                    <span class="font-medium text-gray-900">Personalizzazione (+{{ number_format(config('toimeme.customization_price'), 2, ',', '.') }} € / conf.)</span>
                </span>
            </div>
        @endif
    </div>
